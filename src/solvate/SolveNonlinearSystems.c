 ...
 * and variable types changed ACRM 16.09.02
 *
 */
...
long
SolveNonlinearSystem(
	REAL			(**func)(REAL*),	/* Vector of functions */
	REAL			*x,					/* Vector of independent variables */
	register long	n,					/* Order of system */
	REAL			numsig,				/* Number of significant decimal digits */
	REAL			*maxit				/* Maximum number of iterations; return actual number */
)
{
	register long i, k;
	long converge, m, j, jsub, itemp, kmax, kplus, tally;
	REAL f, hold, h, dermax, test, factor, relconvg;
	long pointer[MAXN * MAXN], isub[MAXN - 1];
	REAL lastx[MAXN], part[MAXN], coe[MAXN * (MAXN + 1)];

	converge = 0;
	relconvg = pow(10.0, (double)(-numsig));
	for (m = 1; m <= *maxit; m++) {
#ifdef DEBUG
           fprintf(stderr,"Iter %ld Vec: %.3f %.3f %.3f\n", m, x[0], x[1], x[2]);
#endif
